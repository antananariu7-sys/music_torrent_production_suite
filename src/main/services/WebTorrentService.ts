...
import { readFileSync, writeFileSync, existsSync, mkdirSync, statSync, rmSync, readdirSync } from 'fs'
...
export class WebTorrentService {
   ...
   */
  private cleanupDeselectedFiles(qt: QueuedTorrent): void {
        ...
        if (qt.selectedFileIndices && qt.selectedFileIndices.length > 0) {
          ...
          if (qt.downloadPath) {
            ...
            if (allExist) {
              ...
              this.persistQueue()
              this.broadcastStatusChange(qt)
              this.cleanupDeselectedFiles(qt)
              this.processQueue()
              return
            }
          }
