...
export function registerWaveformHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event, request: unknown) => {
      try {
        const { songId, filePath } =
          WaveformGenerateRequestSchema.parse(request)
  ...
  ipcMain.handle(
    IPC_CHANNELS.WAVEFORM_GENERATE_BATCH,
    async (_event, request: unknown) => {
      try {
        ...
        console.error(
          '[waveformHandlers] Failed to generate batch waveforms:',
          error
        )
        return {
          success: false,
          error:
            error instanceof Error
              ? error.message
              : 'Failed to generate batch waveforms',
        }
      }
    }
  )
  ...
  ipcMain.handle(
    IPC_CHANNELS.WAVEFORM_REBUILD_BATCH,
    async (_event, request: unknown) => {
      try {
        const { projectId } = WaveformBatchRequestSchema.parse(request)
        await waveformExtractor.invalidateAllCaches()
        const data = await waveformExtractor.generateBatch(projectId)
        return { success: true, data }
      } catch (error) {
        console.error('[waveformHandlers] Failed to rebuild waveforms:', error)
        return {
          success: false,
          error:
            error instanceof Error
              ? error.message
              : 'Failed to rebuild waveforms',
        }
      }
    }
  )
