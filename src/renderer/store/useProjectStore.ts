...
export const useProjectStore = create<ProjectState>()(
  temporal(
    (set) => ({
      ...
      loadRecentProjects: async () => {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            set({
              error: response.error || 'Failed to load recent projects',
              isLoading: false,
            })
          }
        } catch (error) {
      ...
      openProject: async (filePath: string) => {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            set({
              error: response.error || 'Failed to open project',
              isLoading: false,
            })
          }
        } catch (error) {
      ...
      deleteProjectFromDisk: async (
        projectId: string,
        projectDirectory: string
      ) => {
        try {
          const response = await window.api.deleteProjectFromDisk(
            projectId,
            projectDirectory
          )
          if (response.success) {
            ...
            set({
              error: response.error || 'Failed to delete project from disk',
            })
            return false
          }
        } catch (error) {
    ...
    }),
    {
      ...
      equality: (pastState, currentState) =>
        pastState.currentProject === currentState.currentProject,
    }
  )
)
