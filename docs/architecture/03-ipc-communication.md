...
// Renderer usage
const result = await window.api.createProject({ name: 'My Project', directory: '/path' })
```
...
ipcMain.handle(IPC_CHANNELS.SEARCH_START_PROGRESSIVE, async (event, request) => {
  // Periodically push progress to the renderer
  event.sender.send(IPC_CHANNELS.SEARCH_PROGRESS, { page: 1, totalPages: 5, resultsCount: 50 })
