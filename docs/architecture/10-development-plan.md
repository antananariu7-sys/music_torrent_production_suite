...
**Phase 2: Component 1 - Search Engine (Week 3-4)** ✅ Core Complete
...
**Phase 2b: Search Results Table Overhaul** ✅ Complete

- ✅ Data-dense sortable table replacing card layout (SearchResultsTable, SearchResultsRow)
- ✅ Sortable columns with 3-click cycle, collapsible group headers
- ✅ Album / Discography tabs with independent state (SearchResultsTabs)
- ✅ Row expansion with lazy-loaded track list preview
- ✅ Client-side text filter with title highlighting + pagination (20/50/100)
- ✅ Non-audio result exclusion + FLAC image badges
- ✅ Discography match column with sort priority + expanded row highlighting
- ✅ Load more results — on-demand page fetching via `search:load-more` IPC
- ✅ Duplicate detection — background check against project audio files
- See plan: `docs/features/done/plans/search-results-table-plan.md`
...
**Phase 4b: Component 3b - Audio Mix Export** ✅ Phase 1 Complete

- ✅ Add `ffmpeg-static` dependency, configure esbuild externals
- ✅ `MixExportService`: validate → analyze loudness → render → cue sheet
- ✅ IPC channels: `mix-export:start`, `mix-export:progress`, `mix-export:cancel`, `mix-export:ffmpeg-check`
- ✅ Crossfade controls between tracklist rows
- ✅ Export config modal (format, quality, output path, normalization toggles)
- ✅ Background rendering with progress bar and cancel
- ✅ Cue sheet generation with accurate timestamps
- ✅ Unit tests: FilterGraphBuilder, CueSheetGenerator, MixValidator
- ⏳ Phase 2: Zod validation, chunked rendering, accessibility, config persistence
